...
本文是当前五层基线中的 Core 规范文档。`02-core_shell_profile_architecture_spec.md` 负责描述 Protocol/Core/Shell/App/Profile 的整体分层与主线路径；本文只聚焦 Core 层内部语义、状态推进、验收、失败吸收与恢复规则。本文描述的是当前实现态与规范边界，不是未来完整 agent 平台的总设计。阅读本文时应结合 glossary 与 runtime visual model 交叉对照。
...
6. **Semantic Separation**：必须区分 task verification、milestone acceptance、goal acceptance、review-required、retry、repair、replan、terminal failure，并避免把这些运行语义压缩成单一 success/failure 二元结果。
...
canonical loop 在当前 runtime 语义下，更准确地保持为：

`Plan -> Dispatch -> Execute -> Absorb Result -> Verify -> Recover / Review / Replan / Terminate`

说明：

- `Execute` 表示 Core 发出 effect request，并等待 Shell 回流 normalized result；
- `Absorb Result` 表示 Core 吸收 `EffectResult` 并更新状态；
- `Verify` 表示 Core 对 task / milestone / goal 的验收语义进行判断；
- `Recover / Review / Replan / Terminate` 表示 Core 在失败、验收不足、需人工确认或无可继续路径时进入相应语义分支。

这样表达更符合当前五层基线下的 runtime 闭环，也比线性 `Execute -> Verify -> Repair` 更接近健壮 runtime 的真实语义。
...
6. **Failure absorption is part of the main runtime path**：失败结果、验收不足、review-required 与预算耗尽等，不应被视为主流程之外的异常角落；它们属于 Core 必须显式处理的运行语义分支。  
7. **Robust runtime requires visible recovery semantics**：若运行时允许继续、重试、补救、升级审查或重规划，这些路径都应在 Core 语义中可见，而不应只隐藏在 Shell 或调用方约定中。
